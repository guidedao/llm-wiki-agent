...
Нужен Python `3.11` и установленный `uv`. Если `uv` не найден, установите его по
официальной инструкции Astral или через менеджер пакетов вашей системы, затем
заново откройте терминал. Все команды ниже запускайте из корня репозитория,
где лежат `README.md` и `pyproject.toml`.
...
маршрута майлстоунов M2/M3. Старый `m0_query.json` оставлен только для
совместимости ранних заметок и тестов.

`M` в названиях `M0`, `M1`, `M2` и `M3` означает milestone, то есть учебную
веху проекта. Это не git-тег, не ветка и не скрытая стартовая точка.
...
| `uv run kb-agent --query-fixture fixtures/queries/m2_query.json --vault-root vault` | собирает локальный wiki-путь без API-ключа | чтобы увидеть артефакты запуска и проверить майлстоуны M2/M3 | появился `run_id`, ответ, сводка и отчёт проверки состояния |
...
- **Reserved capacity** — заранее выделенная GPU-capacity под клиента или класс
  нагрузки.
- **Burst** — временный пик нагрузки сверх обычного уровня.
- **Buffer** — запас capacity, который помогает не сорвать обещания клиентам.
- **Queue** — очередь задач или запусков, ожидающих доступного ресурса.
- **Margin** — разница между выручкой и затратами на capacity.
- **Aurora-17** — учебный инцидент Northstar Compute, где один статус очереди
  скрывал несколько причин ожидания.

## Что Сейчас Реализовано

Текущий каркас покрывает базовый срез майлстоунов `M0 + M1 + M2 + M3` на
корпусе Northstar Compute:
...
Кэпстоун намеренно не требует Agents SDK в базовом пути. Мы руками показываем
те же контракты, которые потом удобно узнавать в SDK: состояние запуска,
контракты тулов, трейсы, границы чтения/записи и handoff-заметку. Это учебный
харнес, а не курс по одному фреймворку.

В базовом LMS-сценарии отдельного майлстоуна `M4` нет: финальная сдача строится
вокруг чтения одного `run_id`, артефактов майлстоунов `M2/M3`, eval-отчёта и
короткой инженерной заметки.
