...
полной LLM Wiki в том, что агент постепенно поддерживает **накапливающийся
артефакт знаний**: читает новые источники, обновляет связанные страницы,
замечает противоречия, ведёт индекс и оставляет журнал изменений.

Базовый срез кэпстоун-проекта уже намеренно проще. Он учит безопасному маршруту:
прочитать raw-источники, собрать производный wiki-слой, выбрать пакет контекста,
ответить с опорой на источники и оставить проверяемые артефакты одного запуска.
Автономная обратная запись в устойчивую wiki остаётся optional-расширением после
базовой сдачи.
...
После `kb-agent` скопируйте `run_id` из вывода команды. Это номер конкретного
запуска агента. Не путайте его с `eval_run_id`, который появляется
...
- **Eval-отчёт** — маленькая retrieval/grounding smoke-проверка: нашёл ли
  агент ожидаемые источники и умеет ли отказаться при недостатке контекста.
...
- retrieval/grounding smoke eval на несколько сценариев привязки к источникам и сбоя;
- live-путь ответа через OpenAI Responses API.

Кэпстоун намеренно не требует Agents SDK в базовом пути. Мы руками показываем
те же идеи, которые потом удобно узнавать в SDK: состояние запуска, контракты
