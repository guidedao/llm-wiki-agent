...
- runtime, который пишет состояние запуска, трейсы, health-отчёт и markdown-выводы;
...
- health checks;
- дальнейший controlled write-back.
...
Сейчас scaffold реализует узкий путь `M0 + M2 + ранний M3`:
...
- сохраняет run record, JSONL trace и health report для запуска;
- держит читаемый `run_budget.yaml` для основных runtime-стадий.
...
- `M3`: более богатое состояние runtime, operator remediation и recovery exercises;
