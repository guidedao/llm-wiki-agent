...
## Мини-лор для ранних примеров

Некоторые ноутбуки используют учебный мир кэпстоун-проекта до того, как студент
полностью погрузился в финальный проект.

Достаточно знать следующее: **Northstar Compute** — вымышленная
AI-инфраструктурная компания, которая продаёт управляемую GPU-capacity для
training- и inference-нагрузок. **Aurora-17** — учебный инцидент в её
GPU-кластере: по нему удобно показывать, как агент выбирает контекст, отличает
источник от инструкции и собирает проверяемый ответ.
