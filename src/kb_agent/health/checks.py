...
def build_health_report(artifacts_dir: Path, run_id: str) -> dict:
    ...
    answer_source = run_record.get("answer_source")
    openai_metadata_path = run_record.get("openai_response_metadata_path")
    summary_path = run_record.get("summary_path")
    ...
    checks = [
        ...
        _check(
            ...
            not missing_events,
            "не хватает=" + ", ".join(missing_events)
            if missing_events
            else "полный набор событий",
        ),
        _check(
        ...
        _check(
            "run_summary_exists",
            bool(summary_path and Path(summary_path).exists()),
            str(summary_path),
        ),
    ]
    if answer_source == "openai_responses":
        metadata_path = Path(openai_metadata_path) if openai_metadata_path else None
        checks.append(
            _check(
                "openai_response_metadata_exists",
                bool(metadata_path and metadata_path.exists()),
                str(openai_metadata_path),
            )
        )
