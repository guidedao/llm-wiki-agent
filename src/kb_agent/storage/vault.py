...
def write_run_summary(
    ...
    *,
    ...
    lines = [
        ...
        "- Source-map показывает, какие raw-источники поддерживают ответ.",
        "- Пакет контекста показывает, какой контекст был выбран.",
        "- План показывает промежуточные шаги перед ответом.",
        "- Контракты тулов показывают границу между чтением и записью.",
...
def _render_log_header() -> str:
    return "\n".join(
        [
            "# Журнал запусков",
            "",
            "Здесь фиксируются учебные запуски и их главные артефакты.",
            "",
        ]
    )
